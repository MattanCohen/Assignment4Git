...
def output_check():
    try:
        cmp = np.array([[str(x) for x in (l.strip('\n').split(','))] for l in open(sys.argv[1])]) == np.array(
            [[str(x) for x in (l.strip('\n').split(','))] for l in open(sys.argv[2])])
        [print(f'Mismatch in line {i} value {j}') for i, j in
         itertools.product(range(cmp.shape[0]), range(cmp.shape[1])) if cmp[i, j] == False]
